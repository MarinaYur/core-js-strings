...
function countVowels(str) {
  ...
  return str1.split('').filter((item) => vowels.includes(item)).length;
  // throw new Error('Not implemented');
}
...
function isPalindrome(str) {
  const str1 = str.toLowerCase().match(/\w/g).join('');
  const strReverce = str1.split('').reverse().join('');
  return str1 === strReverce;
  // throw new Error('Not implemented');
}
...
function findLongestWord(sentence) {
  ...
  for (let i = 0; i < arrSent.length; i += 1) {
    if (arrSent[i].length > longWord.length) {
      longWord = arrSent[i];
    }
  }
  return longWord;
...
function reverseWords(str) {
  ...
  return arrStr.map((item) => item.split('').reverse().join('')).join(' ');
  // throw new Error('Not implemented');
}
...
function encodeToRot13(str) {
  ...
  const newAlf = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  let result = '';
  const sign = alf.includes(str[str.length - 1]) ? '' : str[str.length - 1];
  for (let i = 0; i < str.length; i += 1) {
    result += newAlf[alf.indexOf(str[i])] || ' ';
  }
  return result.trimEnd() + sign;
  // throw new Error('Not implemented');
}
...
function getCardId(value) {
  ...
  return arrOfCards.indexOf(value);
  // throw new Error('Not implemented');
}
